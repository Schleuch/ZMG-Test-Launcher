...
ZMG Test Launcher
=================
Launches DayZ (DayZDiag_x64.exe) locally as server + client with -filePatching,
so mods can be tested straight from drive P: —
no PBO packing, signing or copying to the online server.

Requires: Python 3.8+ (standard library only, no pip needed)
"""
...
# Script folder (changes with every new version)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# Fixed data folder – survives version changes
...
# Default base folder for profiles and missions
DEFAULT_BASE = r"C:\ZmG_Test_Launcher"

# Mod list: columns and visible rows (the rest scrolls)
MOD_COLS = 3
MOD_ROWS = 10
MOD_MAX_RENDER = 150      # more matches are not drawn at once

# Colour scheme (dark, DayZ style)
C_BG = "#17181b"        # window background
C_PANEL = "#22242a"     # buttons / panels
C_FIELD = "#2a2d33"     # input fields
C_FG = "#d6d7d9"        # text
C_MUTED = "#9aa0a6"     # muted text
C_ACCENT = "#d98e2b"    # DayZ amber
C_ACCENT2 = "#efb45a"   # light amber (hover)
C_BORDER = "#3a3d44"    # borders
C_RED = "#a03c3c"       # stop/danger

# Folders on P: that are not mods (vanilla data / tools)
IGNORE_DIRS = {"dz", "scripts", "bin", "core", "languagecore", "temp",
               "editor", "mikero", "gui", "worlds_data", "$logs$"}

# Patterns for log colouring
...
# File types watched by the watchdog
WATCH_EXT = (".c", ".cpp", ".hpp", ".layout", ".xml", ".json")

# Detects file + line number in error lines, e.g.
#   P:\ZMG_Mod\scripts\4_World\foo.c(123)  or  scripts/3_Game/bar.c:45
...
BattlEye = 0;                // IMPORTANT: 0 = BattlEye off (local test)
verifySignatures = 0;        // IMPORTANT: 0 so unsigned mods can load
allowFilePatching = 1;       // IMPORTANT: 1, otherwise the server ignores -filePatching
forceSameBuild = 0;
...
class Launcher(tk.Tk):
    def __init__(self):
        ...
        if getattr(self, "_migrated", False):
            self.log("[Launcher] Settings taken over from the previous "
                     f"version ({OLD_CONFIG_FILE}).")
            self.log(f"[Launcher] Data folder is now version independent: {APP_DIR}")
            self.save_config()
    ...
    def _dark_titlebar(win):
        """Switch the Windows title bar to dark (Win10 1809+, no-op otherwise)."""
        try:
            import ctypes
    ...
    def load_config(self):
        ...
        if not os.path.isfile(CONFIG_FILE) and os.path.isfile(OLD_CONFIG_FILE):
            source = OLD_CONFIG_FILE        # take over settings from the previous version
        try:
        ...
        # Sensible derivations for the first start (only if field is empty):
    ...
    def save_config(self):
        ...
        try:
            ...
            self.log(f"[Launcher] Could not save config: {e}")

    # --------------------------------------------------------------- profile
    def exp_defaults(self):
        """Suggested values for an experimental profile."""
    ...
    def seed_required_mods(self):
        """Add known required mods to profiles that have none."""
        for name, data in self.cfg.get("profiles_sets", {}).items():
            ...
            for m in req:
                ...
                added += 1
            if added:
                self.log("[Mods] profile %s: added %d required mod(s)." % (name, added))

    def normalize_profile_cfgs(self):
        """Give every profile its own server config (no duplicates)."""
        sets = self.cfg.get("profiles_sets", {})
        used = set()
        for name in sorted(sets):
            ...
            for cand in candidates:
                ...
                used.add(nkey)
                self.log("[Launcher] profile %s gets its own config: %s" % (name, cand))
                break
    ...
    def switch_profile(self, event=None):
        ...
        self.save_config()
        self.scan_mods()
        self.log("[Launcher] switched profile: " + name)
    ...
    def new_profile(self):
        name = simpledialog.askstring("New profile", "Profile name.\n\nIt automatically gets its own mission and profiles folder plus a free port.\nIf the name contains 'Exp', experimental paths are used.",
                                      initialvalue="", parent=self)
        if not name:
            return
        name = name.strip()
        if name in self.cfg.get("profiles_sets", {}):
            messagebox.showinfo("Profile", "This profile already exists.")
            return
        self.cfg.setdefault("profiles_sets", {})[
        ...
        self.save_config()
        self.scan_mods()
        self.log("[Launcher] profile created: " + name)

    def delete_profile(self):
        name = self.cb_profile.get()
        sets = self.cfg.get("profiles_sets", {})
        if len(sets) < 2:
            messagebox.showinfo("Profile", "The last profile cannot be deleted.")
            return
        if not messagebox.askyesno("Delete profile", "Really delete this profile: " + name):
            return
        sets.pop(name, None)
        nxt = sorted(sets)[0]
    ...
    def build_ui(self):
        ...
        ttk.Label(frm_prof, text="Profile:").pack(side="left")
        ...
        ttk.Button(frm_prof, text="New",
                   command=self.new_profile).pack(side="left", padx=3)
        ttk.Button(frm_prof, text="Delete",
                   command=self.delete_profile).pack(side="left")
        ttk.Button(frm_prof, text="Required mods",
        ...
        frm_paths = ttk.LabelFrame(self, text="Paths & Settings")
        ...
        self.e_dayz = row(0, "DayZ folder (server):", "dayz_path")
        self.e_client = row(1, "DayZ folder (client, optional):", "client_path")
        self.e_pdrive = row(2, "Drive P (mod source):", "p_drive")
        self.e_workshop = row(3, "Workshop folder (!Workshop):", "workshop_path")
        self.e_servercfg = row(4, "serverDZ.cfg:", "server_cfg", browse_dir=False,
                               filetypes=[("cfg", "*.cfg"), ("All", "*.*")])
        ttk.Button(frm_paths, text="Create",
                   command=self.create_server_cfg).grid(row=4, column=3, **pad)
        self.e_mission = row(5, "Mission folder (server):", "mission_path")
        ttk.Button(frm_paths, text="Create",
                   command=self.download_missions).grid(row=5, column=3, **pad)
        self.e_profiles = row(6, "Profiles folder (logs):", "profiles_path")
        ...
        ttk.Label(sub, text="   Extra parameters:").pack(side="left")
        ...
        # Mods
        frm_mods = ttk.LabelFrame(self, text="Available mods:")
        frm_mods.pack(fill="x", padx=8, pady=4)
        top = ttk.Frame(frm_mods)
        top.pack(fill="x")
        ttk.Button(top, text="Rescan", command=self.scan_mods).pack(side="left", padx=6, pady=4)
        ttk.Button(top, text="＋ PBOs as mod", command=self.add_pbos).pack(side="left", padx=(14, 3))
        ttk.Button(top, text="＋ Mod folder", command=self.add_mod_folder).pack(side="left")
        ...
        ttk.Button(top, text="Save", command=self.save_preset).pack(side="left", padx=3)
        ttk.Button(top, text="Delete", command=self.delete_preset).pack(side="left")
        ttk.Button(top, text="Load order",
                   command=self.open_load_order).pack(side="left", padx=(14, 0))
        self.b_vpp = ttk.Button(top, text="VPP Settings",
                                command=self.open_vpp_settings)
        # only shown while a VPP mod is checked
        ...
        self.v_src = {}
        for key, text in (("ws", "Workshop"), ("p", "Drive P"),
                          ("pbo", "PBOs"), ("dir", "Mod folders")):
        ...
        ttk.Checkbutton(srow, text="Active only", variable=self.v_only_active,
        ...
        # scrollable area for the mod checkboxes
        ...
        self.b_server = ttk.Button(frm_btn, text="▶ Start server",
                                   style="Accent.TButton", command=self.start_server)
        self.b_server.pack(side="left", padx=4)
        self.b_client = ttk.Button(frm_btn, text="▶ Start client",
                                   style="Accent.TButton", command=self.start_client)
        self.b_client.pack(side="left", padx=4)
        ttk.Button(frm_btn, text="▶▶ Start both", style="Accent.TButton",
                   command=self.start_both).pack(side="left", padx=4)
        ttk.Button(frm_btn, text="■ Stop all", style="Stop.TButton",
                   command=self.stop_all).pack(side="left", padx=12)
        ttk.Button(frm_btn, text="🗑 Wipe storage", command=self.wipe_storage).pack(side="left", padx=4)
        ...
        ttk.Checkbutton(frm_btn, text="Auto-restart on change",
        ...
        ttk.Checkbutton(frm_btn, text="Client without diag",
                        variable=self.v_no_diag).pack(side="left", padx=(14, 0))
        self.v_wrap = tk.BooleanVar(value=bool(self.cfg.get("wrap_sources", True)))
        ttk.Checkbutton(frm_btn, text="Auto-pack P: sources",
                        variable=self.v_wrap).pack(side="left", padx=(14, 0))
        self.lbl_status = ttk.Label(frm_btn, text="Server: off   |   Client: off")
        self.lbl_status.pack(side="right", padx=6)

        # Quick settings (write directly into mission/server config)
        ...
        ttk.Button(frm_quick, text="Set",
                   command=self.set_login_timer).pack(side="left", padx=(3, 16))
        ttk.Label(frm_quick, text="Server time at start (hour, empty = system time):"
        ...
        ttk.Checkbutton(frm_quick, text="Delete PBOs after testing",
                        variable=self.v_temp_pbo).pack(side="right", padx=6)
        self.b_cache = ttk.Button(frm_quick, text="Clear PBO cache",
                                  command=self.clear_pbo_cache)
        self.b_cache.pack(side="right", padx=4)

        # Log
        frm_log = ttk.LabelFrame(self, text="Server log (newest *.RPT in the profiles folder)")
        frm_log.pack(fill="both", expand=True, padx=8, pady=6)
        logtop = ttk.Frame(frm_log)
        logtop.pack(fill="x")
        self.v_only_err = tk.BooleanVar(value=False)
        ttk.Checkbutton(logtop, text="Errors only", variable=self.v_only_err,
                        command=self.re_render_log).pack(side="left", padx=6, pady=2)
        ttk.Button(logtop, text="Clear log",
                   command=self.clear_log).pack(side="left", padx=6)
        ttk.Button(logtop, text="📂 Open profiles folder",
    ...
    def scan_mods(self):
        self.mod_vars.clear()
        self.mod_source = {}
        self.mod_notes = []          # info lines (e.g. path missing)

        mods = []  # list of (display name, full path); workshop first!

        # 1) Workshop mods (ready @ folders, e.g. @CF, @Dabs Framework)
        ...
        if os.path.isdir(p):
            try:
                ...
                self.mod_notes.append(f"Error scanning P: {e}")
        else:
            self.mod_notes.append(f"Path not found: {p}")

        # 3) Manually added mods ([PBO]: PBO import & external folders)
        extra = []
        seen_local = set()
        ...
        for full, kind in extra:
            tag = "[PBO] " if kind == "pbo" else "[Folder] "
    ...
    def schedule_render(self, delay=220):
        """Debounce redraws - otherwise typing stutters with many mods."""
    ...
    def _mod_cell(self, i):
        """Take a row from the widget pool (or create it)."""
        pool = self.mod_pool
        while len(pool) <= i:
    ...
    def render_mods(self):
        """Draw the mod checkboxes - filtered by the search field."""
        ...
        if not mods:
            notes.append("No mod folders found.")
        elif not shown:
            notes.append("No active mods" if only_active and not query
                         else "No match for: %s" % term)
        elif limited:
            notes.append("Showing only the first %d of %d matches - narrow the search." % (MOD_MAX_RENDER, len(shown)))
        ...
        self.lbl_modcount.config(text="%d/%d shown - %d active" % (len(shown), len(mods), checked))
        self.mod_canvas.yview_moveto(0)

    @staticmethod
    def _workshop_id(folder):
        """Determine a mod folder's Steam Workshop ID (or None)."""
    ...
    def open_mod_link(self, path):
        """Click on a mod name: open the workshop page or the folder."""
        import webbrowser
        ...
        if wid:
            url = ("https://steamcommunity.com/sharedfiles/filedetails/?id="
                   + wid)
            webbrowser.open(url)
            self.log(f"[Mods] Workshop page opened: {url}")
        else:
            self.log(f"[Mods] No workshop ID – opening folder: {path}")
    ...
    def _pbo_prefix(src, fallback):
        """Read the prefix from $PBOPREFIX$, otherwise use the folder name."""
    ...
    def build_pbo(self, src, out_path, prefix):
        """Write a folder as an uncompressed PBO (AddonBuilder-compatible)."""
        import hashlib
        import struct
    ...
    def wrap_source(self, path):
        """Pack a P: source into _filePatching\\@Name\\addons\\<prefix>.pbo."""
        ...
        for folder_name, target in entries:
            ...
            # remove leftovers from the junction version
            ...
            if not self._needs_pack(target, pbo):
                self.log(f"[Mods] up to date, no repack: {prefix}.pbo")
                continue
            try:
                count, size = self.build_pbo(target, pbo, prefix)
            except (OSError, ValueError) as e:
                self.log(f"[Mods] ERROR packing {target}: {e}")
                return path
            self.log(f"[Mods] packed: {target} -> {pbo} "
                     f"({count} files, {size / 1048576:.1f} MB, "
                     f"prefix={prefix})")
    ...
    def update_cache_label(self):
        try:
            ...
            return
        self.b_cache.config(text=f"Clear PBO cache ({mb:.0f} MB)"
                            if mb >= 1 else "Clear PBO cache")

    def clear_pbo_cache(self, quiet=False):
        """Delete the generated PBOs in the _filePatching folder."""
        stage = self._stage_dir()
        if not os.path.isdir(stage):
            if not quiet:
                self.log("[Mods] No PBO cache present.")
            return
        if (self.server_proc and self.server_proc.poll() is None) or \
            ...
            if not quiet:
                messagebox.showinfo("PBO cache",
                                    "Please stop server and client first.")
            return
        freed = self._stage_size()
        try:
            shutil.rmtree(stage)
        except OSError as e:
            self.log(f"[Mods] Could not clear the cache: {e}")
            return
        self.log(f"[Mods] PBO cache cleared ({freed / 1048576:.0f} MB freed).")
        self.update_cache_label()

    # ------------------------------------------------------------ load order
    def ordered_chosen(self):
        """Selected mods in the current load order."""
    ...
    def open_load_order(self):
        ...
        if not chosen:
            messagebox.showinfo("Load order", "No mod is selected.")
            return
        win = tk.Toplevel(self)
        win.title("Load order – the top entry loads first")
        win.geometry("640x520")
        win.configure(bg=C_BG)
        self._dark_titlebar(win)

        ttk.Label(win, text="Frameworks (CF, Dabs) belong at the top, "
        ...
        def auto():
            # workshop first, then P: sources, then PBOs/folders
        ...
        ttk.Button(side, text="▲ move up",
                   command=lambda: move(-1)).pack(side="left", padx=3)
        ttk.Button(side, text="▼ move down",
                   command=lambda: move(1)).pack(side="left", padx=3)
        ttk.Button(side, text="Sort automatically",
        ...
        def save():
            ...
            self.save_config()
            self.log("[Mods] load order saved: " + ", ".join(
                self.mod_labels.get(p, os.path.basename(p)) for p in paths))
            win.destroy()

        ttk.Button(side, text="Apply", style="Accent.TButton",
                   command=save).pack(side="right", padx=3)
        refresh(0)

    def mod_param(self):
        # Order = order in the list (workshop/frameworks first)
        chosen = []
    ...
    def _workshop_name(folder):
        """Read the mod name from a workshop ID folder's meta.cpp."""
        meta = os.path.join(folder, "meta.cpp")
        try:
            with open(meta, "r", encoding="utf-8", errors="replace") as f:
    ...
    def add_pbos(self):
        files = filedialog.askopenfilenames(
            title="Select PBO files",
            filetypes=[("PBO", "*.pbo"), ("All files", "*.*")])
        if not files:
            return
        default = os.path.splitext(os.path.basename(files[0]))[0]
        name = simpledialog.askstring(
            "Mod name", "Name for the new mod folder:",
        ...
        for f in files:
            try:
                ...
                copied += 1
                # take matching signature files along (harmless locally)
                ...
                return
        self.log(f"[Launcher] Copied {copied} PBO(s) to {name}\\addons.")
        self.save_config()
        self.scan_mods()
    ...
    def add_mod_folder(self):
        ...
        # Sanity check: does the folder (directly or in addons/) contain PBOs?
        ...
        if not has_pbo and not messagebox.askyesno(
                "No PBOs found",
                "No .pbo files were found in this folder.\n"
                "Add it anyway?"):
            return
        ...
        self.log(f"[Launcher] Mod folder added: {p}")
    ...
    def remove_extra_mod(self, path):
        ...
        if in_local:
            if messagebox.askyesno(
                    "Remove mod",
                    f"Delete {os.path.basename(path)} from LocalMods?\n"
                    "(The copied PBOs will be deleted)"):
                try:
                    shutil.rmtree(path)
                except OSError as e:
            ...
            if not messagebox.askyesno(
                    "Remove mod",
                    f"Remove {os.path.basename(path)} from the list?\n"
                    "(The folder itself is left untouched)"):
                return
            if path in self.cfg.get("extra_mods", []):
                self.cfg["extra_mods"].remove(path)
        self.save_config()
        self.scan_mods()
        self.log(f"[Launcher] Removed: {path}")
    ...
    def server_exe(self):
        ...
        # 1. diag exe in the server folder
        exe = self._find_exe(folder, ("DayZDiag_x64.exe",))
        if exe:
            return exe
        # 2. diag from the client folder – runs with -server and without BattlEye
        client = self.e_client.get().strip()
        if client:
            exe = self._find_exe(client, ("DayZDiag_x64.exe",))
            if exe:
                self.log("[Launcher] no diag exe in the server folder – using "
                         f"the diag from the client folder ({exe}). "
                         "This runs the test without BattlEye.")
                return exe
        # 3. retail server (BattlEye active!)
        exe = self._find_exe(folder, ("DayZServer_x64.exe",))
        if exe:
            self.log("[Launcher] WARNING: starting a retail server – BattlEye "
                     "is active and will kick local clients (0x000400F0). "
                     "Better use a DayZDiag_x64.exe.")
            return exe
        messagebox.showerror(
            "Server exe missing",
            "Neither DayZDiag_x64.exe nor DayZServer_x64.exe was found in\n"
            f"the DayZ server folder:\n{folder}\n\n"
            "Please check the path (server installation or a client with\n"
            "the diag exe).")
        return None

    def server_is_diag(self):
        """Does the server run a diag exe?"""
        folder = self.e_dayz.get().strip()
        client = self.e_client.get().strip()
    ...
    def client_exe(self):
        ...
        # client and server must match (error 0x00020017)
        if not no_diag and not self.server_is_diag() \
                and self._find_exe(folder, ("DayZ_x64.exe",)):
            no_diag = True
            self.log("[Launcher] server runs without diag – starting the "
                     "client without diag too (otherwise error 0x00020017).")
        elif no_diag and self.server_is_diag() \
                and self._find_exe(folder, ("DayZDiag_x64.exe",)):
            no_diag = False
            self.log("[Launcher] server runs with diag – starting the client "
                     "with diag too.")
        if no_diag:
            names = tuple(reversed(self.CLIENT_EXES))   # DayZ_x64.exe first
        exe = self._find_exe(folder, names)
        if exe:
            return exe
        messagebox.showerror(
            "Client exe missing",
            "Neither DayZDiag_x64.exe nor DayZ_x64.exe was found in\n"
            f"the client folder:\n{folder}\n\n"
            "Tip: enter the game folder in 'DayZ folder (client, optional)'.\n"
            "The diag exe comes via Steam > DayZ > Properties > Betas.")
        return None
    ...
    def create_server_cfg(self):
        ...
        self.log(f"[Launcher] Template created: {path}")
        messagebox.showinfo("Created",
                            "serverDZ_test.cfg has been created.\n"
                            "verifySignatures=0 and allowFilePatching=1 are set.")
    ...
    def _insert_line(self, line, tag):
        """Insert a line into the log widget and make file references clickable."""
    ...
    def log(self, line):
        ...
        if len(self.log_lines) > 20000:            # cap memory usage
    ...
    def resolve_path(self, raw):
        """Resolve a path from an error line into a real file path."""
        ...
        if os.path.isfile(cand):
            return cand
        # last resort: search the file name in the checked P: mods
    ...
    def open_source_popup(self, path, lineno):
        ...
        ttk.Button(bar, text="Open in VS Code",
                   command=lambda: self.open_in_vscode(real, lineno)).pack(side="right", padx=6)
        ttk.Button(bar, text="Default editor",
        ...
        txt.see(f"{max(1, lineno - 8)}.0")          # target line with context above
        txt.see(f"{lineno}.0")
    ...
    def open_in_vscode(self, path, lineno):
        try:
            ...
            messagebox.showinfo("VS Code",
                                "VS Code ('code') was not found.")

    def open_in_default(self, path):
        try:
            os.startfile(path)                      # Windows
        except (AttributeError, OSError):
            messagebox.showinfo("Editor", "Could not start the default editor.")
    ...
    def set_login_timer(self, quiet=False):
        ...
        try:
            ...
            return False
        self.save_config()
        self.log(f"[Quick] TimeLogin = {val} set ({path}) – "
    ...
    def set_server_time(self, quiet=False):
        ...
        if not os.path.isfile(cfgfile):
            self._quick_report(quiet, False,
                               f"Server time: serverDZ.cfg not found ({cfgfile}).")
            return False
        if not raw or raw.lower() in ("sys", "system", "systemtime"):
            ...
            if not m or not (0 <= int(m.group(1)) <= 23) \
                ...
                self._quick_report(quiet, False,
                                   "Server time: enter hour (0-23) or "
                                   "hour:minute, empty = system time.")
                return False
            hour = int(m.group(1))
        ...
        try:
            ...
            if n == 0:      # line missing entirely -> append
                new_content = content.rstrip() + \
                    f'\nserverTime = "{new_val}";\n'
            with open(cfgfile, "w", encoding="utf-8") as f:
                f.write(new_content)
        except OSError as e:
            self._quick_report(quiet, False, f"Server time: {e}")
            return False
        self.save_config()
        self.log(f'[Quick] serverTime = "{new_val}" set ({cfgfile}) – '
                 "takes effect on the next server start.")
        return True
    ...
    def activate_present_required(self):
        """Tick required mods that are already installed."""
    ...
    def check_required_mods(self, silent=False):
        ...
        names = "\n".join(
            ...
            for m in missing)
        if silent:
            self.log("[Mods] %d required workshop mod(s) missing." % len(missing))
            return False
        if not messagebox.askyesno(
                "Required mods",
                "The following workshop mods are missing for this profile:\n\n%s\n\nOpen them in Steam to subscribe now?" % names):
            return False
        import webbrowser
        for m in missing:
            wid = m.get("id", "").strip()
            ...
            self.log("[Mods] Steam page opened: %s (ID %s)" % (m.get("name") or wid, wid))
        self.log("[Mods] click 'Subscribe' in Steam – the download is tracked here.")
    ...
    def ensure_required_active(self):
        ...
        if missing:
            names = "\n".join("  - " + (m.get("name") or m.get("id", ""))
                               for m in missing)
            messagebox.showerror("Required mods missing", "This map requires mods that are not installed:\n\n%s\n\nThe server will not start without them." % names)
            self.check_required_mods()
            return False
        if inactive:
            names = "\n".join("  - " + (m.get("name") or m.get("id", ""))
                               for m in inactive)
            if messagebox.askyesno("Required mods missing", "The following required mods are installed but not enabled:\n\n%s\n\nEnable them and start now?" % names):
                for m in inactive:
                    self.activate_mod_entry(m)
                return True
            return False
    ...
    def activate_mod_entry(self, entry):
        """Tick a downloaded required mod in the list."""
        wid = entry.get("id", "").strip()
        ...
        for p, var in self.mod_vars.items():
            ...
            if hit:
                if not var.get():
                    var.set(True)
                    self.log("[Mods] enabled automatically: " + (label or base))
                    self.save_config()
                return True
        return False
    ...
    def _finish_download(self, entry):
        ...
        if not self.activate_mod_entry(entry):
            self.log("[Mods] not found in the list yet (click Rescan): " + (entry.get("name") or entry.get("id", "")))
            ...
            for key, m in list(pending.items()):
                ...
                if folder and os.path.isdir(folder):
                    ...
                    if sizes.get(key) == size and size > 0:
                        self.after(0, self.log, "[Mods] download finished: %s (%.0f MB)" % (
                            m.get("name") or wid, mb))
                        self.after(0, self._finish_download, m)
                        pending.pop(key, None)
                        continue
                    if size and abs(size - sizes.get(key, 0)) > 1048576:
                        self.after(0, self.log, "[Mods] downloading: %s (%.0f MB)" % (
                            m.get("name") or wid, mb))
                    sizes[key] = size
        if pending:
            self.after(0, self.log, "[Mods] watching stopped – not all mods arrived.")

    def open_required_mods(self):
        win = tk.Toplevel(self)
        win.title("Required mods")
        win.geometry("640x460")
        win.configure(bg=C_BG)
        self._dark_titlebar(win)
        ttk.Label(win, text="Required mods for this profile. Missing ones can be subscribed\nright away; the download is tracked in the log.", foreground=C_MUTED,
        ...
        def refresh():
            ...
            if not entries:
                ttk.Label(body, text="No required mods added yet.",
                          foreground=C_MUTED).pack(anchor="w", pady=6)
            for i, m in enumerate(entries):
                row = ttk.Frame(body)
                row.pack(fill="x", pady=2)
                present = self._mod_present(m)
                ttk.Label(row, text="installed" if present else "missing",
                ...
                if not present and wid:
                    ttk.Button(row, text="Download", style="Accent.TButton",
                               command=lambda e=m: self._subscribe_one(e, refresh)
                               ).pack(side="right", padx=2)
                elif not present:
                    ttk.Label(row, text="no workshop ID",
        ...
        def add_entry():
            ...
            if wid and not wid.isdigit():
                messagebox.showinfo("Required mods", "The workshop ID may only contain digits.", parent=win)
                return
        ...
        ttk.Button(add, text="Add", command=add_entry).pack(side="left", padx=4)
        ttk.Button(win, text="Close", style="Accent.TButton",
                   command=win.destroy).pack(pady=8)
        refresh()

    def open_workshop_page(self, wid):
        """Open a mod's workshop page in the browser/Steam."""
        import webbrowser
        url = "https://steamcommunity.com/sharedfiles/filedetails/?id=" + wid
        webbrowser.open(url)
        self.log("[Mods] workshop page opened: " + url)
    ...
    def _subscribe_one(self, entry, refresh=None):
        ...
        webbrowser.open("https://steamcommunity.com/sharedfiles/"
                        "filedetails/?id=" + wid)
        self.log("[Mods] workshop page opened: %s (ID %s)" % (entry.get("name") or wid, wid))
        threading.Thread(target=self._watch_downloads,
    ...
    MAP_SOURCES = {
        ...
        "dayzOffline.sakhal": (CE_ZIP, "dayzOffline.sakhal", "Sakhal requires the Frostline DLC."),
        "regular.namalsk": (NAMALSK_ZIP, "Mission Files/regular.namalsk",
                            "Namalsk additionally requires the Namalsk workshop mods - add them under 'Required mods'."),
    }
    ...
    def create_cfg_if_missing(self, cfgfile, mission_name):
        ...
        if os.path.isfile(cfgfile):
            return True
        if not messagebox.askyesno("Create serverDZ.cfg", "There is no server config for this profile yet:\n%s\n\nCreate it from the template now?" % cfgfile):
            return False
        ...
        try:
            ...
            messagebox.showerror("Create serverDZ.cfg", str(e))
            return False
        self.log("[Launcher] server config created: %s" % cfgfile)
        return True

    def ensure_profile_cfg(self):
        """On profile switch: offer to create the missing server config."""
        cfgfile = self.e_servercfg.get().strip()
        mission = self.e_mission.get().strip()
    ...
    def set_cfg_template(self, mission_name):
        ...
        try:
            ...
            self.log("[Map] %s" % e)
            return
        self.log("[Map] serverDZ.cfg template = %s set." % mission_name)
    ...
    def copy_local_mission(self, src_dir, dest):
        """Copy the mission from the server installation (without persistence)."""
        if os.path.isdir(dest) and os.listdir(dest):
            if not messagebox.askyesno(
                    "Copy mission",
                    f"The target folder is not empty:\n{dest}\n\n"
                    "Overwrite its content?"):
                return
        if (self.server_proc and self.server_proc.poll() is None) or \
                (self.client_proc and self.client_proc.poll() is None):
            messagebox.showinfo("Copy mission",
                                "Please stop server and client first.")
            return
        skipped = []
        count = 0
        try:
            ...
            for dirpath, dirs, files in os.walk(src_dir):
                # do not take the server installation's persistence or logs
                ...
                for fn in files:
                    ...
                    try:
                        ...
                        # skip locked files (WinError 32)
                        skipped.append(f"{os.path.join(rel, fn)}  ({e.strerror})")
        except OSError as e:
            messagebox.showerror("Copy mission", str(e))
            return
        if skipped:
            for s in skipped:
                self.log(f"[Mission] skipped: {s}")
            messagebox.showwarning(
                "Copy mission",
                f"{count} files copied, {len(skipped)} locked and skipped.\n\n"
                "Locked files usually come from a running server (including "
                "one started from a batch file). Stop it and click 'Create' "
                "again.\n\nDetails are in the log.")
        self.e_mission.delete(0, "end")
        self.e_mission.insert(0, dest)
        self.save_config()
        self.log(f"[Mission] copied {count} files: {src_dir} -> {dest}")
    ...
    def download_missions(self):
        mission = self.e_mission.get().strip()
        if not mission:
            messagebox.showinfo("Load mission", "Please enter the target path in the 'Mission folder' field first.")
            return
        ...
        if os.path.isfile(os.path.join(local, "init.c")):
            if messagebox.askyesno(
                    "Copy mission",
                    "The mission is part of the server installation:\n%s\n\nIt matches this build exactly. Copy it to:\n%s\n\n(Yes = copy, No = download from GitHub instead)" % (local, mission)):
        ...
        if url is None:
            messagebox.showinfo(
                "Load mission",
                "No download source is configured for '%s'.\n\nPlease place the mission in the folder manually or pick another map via 'Maps'." % folder)
            return
        if not messagebox.askyesno("Load mission", "Download %s from GitHub and extract it to:\n\n%s\n\n(runs in the background, progress in the log below)" % (folder, mission)):
            return
        if hint:
            self.log("[Map] " + hint)
        self.log("[Download] starting download: %s ..." % folder)
        threading.Thread(target=self._download_missions_worker,
                         args=(parent, mission, folder, url, subpath),
                         daemon=True).start()

    def _urlopen(self, url):
        """Open the download connection with SSL fallbacks.

        1. system certificates  2. certifi (if installed)
        3. without certificate verification (with a note in the log)
        """
        import ssl
        import urllib.error
        ...
        try:
            ...
            if not isinstance(getattr(e, "reason", None), ssl.SSLError):
                raise                       # different problem -> report normally
        try:
            import certifi
        ...
        self.after(0, self.log,
                   "[Download] Note: no valid SSL certificates found on this "
                   "system – fetching this download without "
    ...
    def _download_missions_worker(self, target, mission,
        ...
        try:
            with self._urlopen(url or self.MISSION_ZIP_URL) as r, \
                ...
                while True:
                    ...
                    out.write(chunk)
                    read += len(chunk)
                    if read - last >= 25 * 1024 * 1024:     # report every 25 MB
                        last = read
                        self.after(0, self.log,
                                   f"[Download] {read / 1048576:.0f} MB ...")
        except Exception as e:                              # network/HTTP/SSL etc.
            self.after(0, self.log, f"[Download] ERROR while downloading: {e}")
            return

        self.after(0, self.log,
                   f"[Download] {read / 1048576:.0f} MB downloaded, extracting "
                   + folder + " ...")
        ...
        try:
            with zipfile.ZipFile(tmp) as z:
                for info in z.infolist():
                    parts = info.filename.split("/", 1)     # strip repo prefix
                    if len(parts) < 2 or not parts[1]:
                        continue
                    rel = parts[1]
                    if not rel.startswith(prefix):
                        continue
                    inner = rel[len(prefix):]               # path inside the mission
            ...
            self.after(0, self.log, f"[Download] ERROR while extracting: {e}")
            return
        finally:
            try:
                os.remove(tmp)
        ...
        def done():
            self.log(f"[Download] Done – {count} files extracted.")
            self.log(f"[Download] Mission: {mission}")
            self.e_mission.delete(0, "end")
            self.e_mission.insert(0, mission)
            self.save_config()
            # apply login timer and server time to the fresh mission right away
            self.set_login_timer(quiet=True)
            self.set_server_time(quiet=True)
        self.after(0, done)
    ...
    def open_hotreload(self):
        ...
        if not profiles or not os.path.isdir(profiles):
            messagebox.showinfo("Hot reload",
                                "Please set a valid profiles folder first.")
            return

        # collect all JSONs below the profiles folder
        found = []
        ...
        if not found:
            messagebox.showinfo("Hot reload",
                                "No JSON files found in the profiles folder.")
            return

        win = tk.Toplevel(self)
        win.title("JSON hot reload – select files")
        ...
        # scrollable checkbox list, grouped by subfolder
        ...
        last_folder = None
        for rel in found:
            folder = os.path.dirname(rel) or "(profiles root)"
        ...
        ttk.Button(bar, text="Reload selected",
                   command=lambda: self.request_hotreload(
                       profiles, [r for r, v in sel_vars.items() if v.get()], win)
                   ).pack(side="left", padx=8)
        ttk.Label(bar, text="Requires the ZMG_HotReload mod on the server!"
                  ).pack(side="left", padx=6)

    # ------------------------------------------------------------ JSON editor
    # Colours based on VS Code "Dark+"
    JS_BG = "#1e1e1e"
    JS_FG = "#d4d4d4"
    JS_KEY = "#9cdcfe"
    JS_STR = "#ce9178"
    ...
    def open_json_editor(self, path):
        ...
        def do_format():
            try:
                ...
                status.config(text=f"Not valid JSON: {e}", foreground="#f28b82")
                return
            txt.delete("1.0", "end")
            txt.insert("1.0", json.dumps(data, indent=4, ensure_ascii=False))
            highlight()
            status.config(text="Formatted (not saved yet)",
                          foreground=C_ACCENT)
        ...
        def do_save(event=None):
            ...
            try:
                ...
                status.config(text=f"Error line {e.lineno}: {e.msg}",
                              foreground="#f28b82")
                messagebox.showerror(
                    "JSON-Editor",
            ...
            try:
                ...
                return "break"
            status.config(text="Saved: " + path, foreground="#8bc48b")
            self.log(f"[JSON] Saved: {path}")
            return "break"

        ttk.Button(bar, text="Save (Ctrl+S)", style="Accent.TButton",
                   command=do_save).pack(side="left", padx=4)
        ttk.Button(bar, text="Format", command=do_format).pack(side="left", padx=4)
    ...
    def request_hotreload(self, profiles, files, win):
        if not files:
            messagebox.showinfo("Hot reload", "Nothing selected.")
            return
        if not (self.server_proc and self.server_proc.poll() is None):
            if not messagebox.askyesno(
                    "Server not running",
                    "No server is running (via the launcher).\n"
                    "Write the request anyway?"):
                return
        ts = int(time.time())
        ...
        try:
            ...
            messagebox.showerror("Error", str(e))
            return
        self.log(f"[HotReload] Request written ({len(files)} file(s)), "
    ...
    def _poll_reload_response(self, profiles, ts):
        ...
        while time.time() < deadline:
            ...
            return
        self.after(0, self.log,
                   "[HotReload] No response from the server – is the "
                   "ZMG_HotReload mod running and the server up?")
        ...
        if hasattr(self, "lbl_modcount"):
            ...
            if "·" in txt:
                self.lbl_modcount.config(
                    text=txt.split("·")[0] + f"· {checked} active")
    ...
    def _ensure_cf(self):
        ...
        if cf_path:
            if not self.mod_vars[cf_path].get():
                ...
                self.log("[Mods] CF enabled automatically "
                         "(VPPAdminTools requires CF).")
        else:
            messagebox.showwarning(
                "CF missing",
                "VPPAdminTools requires the mod 'CF (Community Framework)'.\n\n"
                "CF was not found in any mod source – please subscribe to it\n"
                "in the Steam Workshop, otherwise VPPAdminTools will not work.")
    ...
    def open_vpp_settings(self):
        ...
        ttk.Label(win, text=(
            "Note: the server (and client) must have been started ONCE\n"
            "with VPPAdminTools enabled so VPP creates its folders in\n"
            "the profiles folder. Save here afterwards."),
    ...
    def _save_vpp(self, win, sid, pw):
        if not sid or not pw:
            messagebox.showinfo("VPP Settings",
                                "Please enter Steam ID and password.",
                                parent=win)
            return
        if not sid.isdigit():
            messagebox.showinfo("VPP Settings",
                                "The Steam ID may only contain digits "
                                "(SteamID64).", parent=win)
            return
        if len(sid) != 17 and not messagebox.askyesno(
                "VPP Settings",
                "A SteamID64 normally has 17 digits.\n"
                f"Your input has {len(sid)}. Save anyway?",
                parent=win):
            return
        ...
        if not os.path.isdir(perm):
            if not messagebox.askyesno(
                    "VPP folder missing",
                    "The folder VPPAdminTools\\Permissions does not exist "
                    "in the profiles folder yet.\n\n"
                    "It is normally created on the FIRST server start with "
                    "VPP.\n\nCreate the folders now anyway and write the "
                    "files?", parent=win):
                return
        ...
        try:
            ...
            messagebox.showerror("VPP Settings", str(e), parent=win)
            return
        self.cfg["vpp_steamid"] = sid
        self.cfg["vpp_password"] = pw
        self.save_config()
        self.log(f"[VPP] SuperAdmins.txt and credentials.txt written "
                 f"({perm}).")
        win.destroy()
    ...
    def apply_preset(self, event=None):
        ...
        for p, v in self.mod_vars.items():
            v.set(p in paths)
        self.log(f"[Launcher] Preset loaded: {name}")

    def save_preset(self):
        name = self.cb_preset.get().strip()
        if not name:
            messagebox.showinfo("Preset", "Please type a name into the preset field first.")
            return
        self.cfg.setdefault("presets", {})[name] = \
            [p for p, v in self.mod_vars.items() if v.get()]
        self.update_preset_list()
        self.save_config()
        self.log(f"[Launcher] Preset saved: {name}")
    ...
    def delete_preset(self):
        ...
        if name in self.cfg.get("presets", {}):
            ...
            self.cb_preset.set("")
            self.save_config()
            self.log(f"[Launcher] Preset deleted: {name}")

    # ----------------------------------------------------------- storage wipe
    def wipe_storage(self):
        if self.server_proc and self.server_proc.poll() is None:
            messagebox.showwarning("Wipe storage",
                                   "Please stop the server first.")
            return
        mission = self.e_mission.get().strip()
        storage = os.path.join(mission, "storage_1")
        if not os.path.isdir(storage):
            messagebox.showinfo("Wipe storage",
                                f"No storage_1 folder found in:\n{mission}")
            return
        if messagebox.askyesno("Wipe storage",
                               "Really delete storage_1?\n"
                               "(Persistence/economy of the test mission will be reset)"):
            try:
                shutil.rmtree(storage)
                self.log("[Launcher] storage_1 deleted – fresh economy on the next start.")
            except OSError as e:
                messagebox.showerror("Error", str(e))
    ...
    def watchdog_worker(self):
        ...
        while True:
            ...
            if snapshot and changed:
                time.sleep(2)                      # brief settle time for multi-file saves
                self.after(0, self.on_source_changed, changed)
                snapshot = {}                      # rebuild fresh after restart
            else:
                snapshot = new_snap

    def on_source_changed(self, changed_file):
        self.log(f"[Watchdog] Change detected: {changed_file}")
        restart_client = self.v_wd_client.get() and \
            self.client_proc and self.client_proc.poll() is None
        self.log("[Watchdog] Restarting server ...")
        self.stop_all()
        self.after(2500, self.start_server)
        if restart_client:
            self.log("[Watchdog] Client restarts in ~12 seconds ...")
    ...
    def check_mods(self):
        """Check the selected mods and report anything odd in the log."""
        chosen = [p for p, v in self.mod_vars.items() if v.get()]
        if not chosen:
            self.log("[Mods] No mod selected – starting without mods.")
            return
        self.log(f"[Mods] passing {len(chosen)} mod(s):")
        classes = {}          # CfgPatches class -> mod
        required = {}         # mod -> required addons
        for p in chosen:
            ...
            if has_pbo:
                ...
                kind = "unpacked source (needs -filePatching)"
            else:
                kind = "!! no config found"
            self.log(f"    - {p}   [{kind}]")

            if os.path.isfile(cpp) and os.path.isfile(binf):
                self.log("        !! config.cpp AND config.bin present – "
                         "the .bin wins, changes to the .cpp have no effect. "
                         "Tip: delete config.bin.")

            if kind.startswith("!!"):
                # look one level deeper: most common mistake (wrong folder)
                try:
                    for sub in os.listdir(p):
                        ...
                        if os.path.isdir(subp) and (
                                ...
                                or os.path.isfile(os.path.join(subp,
                                                               "config.bin"))):
                            self.log(f"        -> config is in: {subp}")
                            self.log("        -> select this subfolder, not "
                                     "the parent folder!")
                            break
                except OSError:
                    pass
            ...
            # evaluate CfgPatches (only possible from a readable config.cpp)
            if os.path.isfile(cpp) and not os.path.isfile(binf):
                ...
                m = re.search(r"class\s+CfgPatches\s*\{(.*?)\n\s*\};",
                              text, re.S)
                if not m:
                    self.log("        !! no CfgPatches block in config.cpp – "
                             "DayZ will not load the mod.")
                    continue
                block = m.group(1)
        ...
        for p, names in required.items():
            ...
            if missing:
                self.log(f"        !! {os.path.basename(p)}: requiredAddons "
                         f"not in the mod list: {', '.join(missing)}")
        self.log("[Mods] check finished. Further down the RPT shows which "
                 "addons were actually loaded.")

    def resolve_mission(self, mission):
        """If the field points at the parent, find the real mission folder."""
        ...
        if len(subs) == 1:
            ...
            self.save_config()
            self.log(f"[Launcher] mission folder detected: {found}")
            return found
        return mission
    ...
    def ensure_cfg_flags(self, cfgfile):
        """Check serverDZ.cfg for the test flags and set them on request."""
        try:
            with open(cfgfile, "r", encoding="utf-8", errors="replace") as f:
                text = f.read()
        except OSError:
        ...
        if not missing:
            return
        names = ", ".join(f"{k} = {v}" for k, v, _ in missing)
        if not messagebox.askyesno(
                "Adjust serverDZ.cfg",
                "The following settings are missing for local testing:\n\n"
                f"{names}\n\n"
                "Without BattlEye = 0 you get kicked on login "
                "(0x000400F0).\n\nSet them now?"):
            return
        ...
        try:
            ...
            self.log(f"[Launcher] serverDZ.cfg not writable: {e}")
            return
        self.log(f"[Launcher] serverDZ.cfg adjusted: {names}")

    def ensure_profiles_inside(self, exe, profiles):
        """BattlEye needs the profiles folder inside the server installation.

        If it sits elsewhere, BE does not initialise and clients get kicked
        with 0x000400F0 (the classic difference to a start batch that uses
        -profiles=config).
        """
        if os.path.basename(exe).lower().startswith("dayzdiag"):
            return profiles            # diag runs without BattlEye
        ...
        if not messagebox.askyesno(
                "Profiles folder and BattlEye",
                "The profiles folder is outside the server installation:"
                f"\n{profiles}\n\n"
                "BattlEye stores its files there and will not start properly "
                "otherwise – the client then gets kicked with 0x000400F0.\n\n"
                f"Use this folder instead?\n{inside}"):
            return profiles
        os.makedirs(inside, exist_ok=True)
        self.e_profiles.delete(0, "end")
        self.e_profiles.insert(0, inside)
        self.save_config()
        self.log(f"[Launcher] profiles folder switched to {inside} "
                 "(BattlEye requirement).")
    ...
    def start_server(self):
        ...
        if not os.path.isdir(mission) or \
            ...
            if messagebox.askyesno("Mission missing", "The mission files for this profile are missing:\n%s\n\nGet them now (copy from the server installation or download from GitHub)?" % mission):
        ...
        if os.path.basename(exe).lower().startswith("dayzdiag"):
            ...
            # retail server: BattlEye needs its own folder, otherwise clients
            # get kicked with 0x000400F0
            bepath = os.path.join(os.path.dirname(exe), "battleye")
            if os.path.isdir(bepath):
                args.append(f"-BEpath={bepath}")
                self.log(f"[Launcher] BattlEye path set: {bepath}")
            else:
                self.log("[Launcher] WARNING: no battleye folder found in the "
                         "server installation – BattlEye may kick clients "
                         "(0x000400F0).")
            self.log("[Launcher] retail server: running without -filePatching (BattlEye would\n                 kick otherwise); mods run as packed PBOs.")
        mp = self.mod_param()
        if mp:
            args.append(mp)
        ...
        self.check_mods()
        self.log("[Launcher] Starting server:\n  " + " ".join(args))
        try:
    ...
    def start_client(self):
        ...
        self.log("[Launcher] Starting client:\n  " + " ".join(args))
        try:
    ...
    def start_both(self):
        self.start_server()
        # start the client slightly delayed so the server claims the port first
        self.after(8000, self.start_client)
        self.log("[Launcher] Client starts in 8 seconds ...")
    ...
    def stop_all(self):
        for name in ("server_proc", "client_proc"):
            ...
            if proc and proc.poll() is None:
                proc.terminate()
                self.log(f"[Launcher] {name} terminated.")
    ...
    def tail_worker(self, profiles, stop):
        # waits for the newest RPT file and tails it
        current = None
        fh = None
        ...
        while not stop.is_set():
            try:
                ...
                if fh:
                    ...
                    self.after(0, self.log,
                               "[Launcher] No RPT file found yet ...")
                    deadline = time.time() + 30
            except OSError:
                pass
            time.sleep(0.4)
    ...
    def watch_processes(self):
        s = "on" if self.server_proc and self.server_proc.poll() is None else "off"
        c = "on" if self.client_proc and self.client_proc.poll() is None else "off"
